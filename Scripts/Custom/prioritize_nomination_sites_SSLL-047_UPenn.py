...
Per requirements:
- Customer & UNCOVERseq inputs MUST have headers: chr, start, stop, strand (strand required).
...
- ON-target naming: If one ONT → OnT_1. If multiple/incongruent ONTs, label by origin
  (OnT_customer_*, OnT_UNCOVERseq_*).
- In-silico ordering (after UNCOVERseq handling): sort by
  levenshtein_distance ↑, then Tier ↑, then MAR with High → Med. → Low.
- UNCOVERseq Top-100 branch:
  If (count(ONT) + count(UNCOVERseq Tiers 1–3)) < 50:
    If (count(ONT) + count(UNCOVERseq ALL Tiers)) < MAX_SITES_RUNTIME:
      add ALL UNCOVERseq (original file order)
    else:
      add FIRST 100 UNCOVERseq rows (original file order)
  Then fill with in-silico and others until MAX_SITES_RUNTIME.
...
Aggregation rules:
 ...
 - Output 'Annot.' is derived from UNCOVERseq 'Annotation', insilico 'gam_region', and 'NONE' for customer
 - Component ordering for aggregation: UNCOVERseq → insilico → customer
...
# Filtering for UMI
# ------------------------------
def filter_by_umi_threshold(df: pd.DataFrame, threshold: float = 0.5) -> pd.DataFrame:
    """
    Filter dataframe to keep only rows where "Avg. % On-Target UMI" > threshold.
    If the column doesn't exist, all rows are kept (threshold not applicable).
    """
    if df.empty:
        return df
    ...
    for col in df.columns:
        col_lower = col.lower()
        if "avg" in col_lower and "on" in col_lower and "target" in col_lower and "umi" in col_lower:
            umi_col = col
            break
    if umi_col is None:
        # Column not found, return df as-is
        return df
    ...
    def to_float_safe(v):
        try:
            ...
            return None
    df_copy = df.copy()
    df_copy['__umi_numeric__'] = df_copy[umi_col].apply(to_float_safe)
    filtered = df_copy[df_copy['__umi_numeric__'] > threshold].drop(columns=['__umi_numeric__'])
    return filtered
...
def load_uncoverseq_csv(path: Optional[str]) -> pd.DataFrame:
    ...
    df["stop"] = df["stop"].apply(_safe_int)
    df["strand"]= df["strand"].astype(str).str.strip()
    # Load Origin column if present
    _ensure_str_col(df, "Origin", "")
...
class Component:
    ...
    uncov_origin: Optional[str] = None
...
def merge_sources(df_customer: pd.DataFrame,
    ...
    def add_row(row: pd.Series, label: str):
        ...
        if label == "UNCOVERseq":
            ...
            # Capture Origin value from UNCOVERseq
            origin_val = str(row.get("Origin", "")).strip() or None
            if origin_val and not rec.uncov_origin:
                rec.uncov_origin = origin_val
    ...
    # Pattern: For ON targets: "Origin_OnT", For OFF targets: "Origin_Tier#_*"
    ...
    for r in all_records:
        tokens: List[str] = []
        if r.is_on:
            # ON target: format is "Origin_OnT"
            if r.uncov_is_on:
                origin_prefix = r.uncov_origin if r.uncov_origin else "UNCOVERseq"
                tokens = [f"{origin_prefix}_OnT"]
            elif r.customer_is_on:
                tokens = ["customer_OnT"]
            elif r.ins_pop_present or r.ins_ref_present:
                origin = "insilico_pop" if r.ins_pop_present else "insilico_ref"
                tokens = [f"{origin}_OnT"]
            else:
                tokens = ["OnT"]
        else:
            # OFF target: format is "Origin_Tier#_*"
            if r.uncov_present:
                tier_val = r.uncov_tier if r.uncov_tier is not None else 9
                x = r.uncov_nonon_index if r.uncov_nonon_index is not None else 0
                origin_prefix = r.uncov_origin if r.uncov_origin else "UNCOVERseq"
                base = f"{origin_prefix}_Tier{tier_val}_{x}"
                tokens = [base]
                ...
                # in-silico only OFF target
                if r.ins_pop_present and r.ins_ref_present:
                    tokens = [f"insilico_pop_Tier{r.ins_tier_min or 0}_{r.ins_pop_index or 0}", 
                              f"insilico_ref_Tier{r.ins_tier_min or 0}_{r.ins_ref_index or 0}"]
                elif r.ins_pop_present:
                    tokens = [f"insilico_pop_Tier{r.ins_tier_min or 0}_{r.ins_pop_index or 0}"]
                elif r.ins_ref_present:
                    tokens = [f"insilico_ref_Tier{r.ins_tier_min or 0}_{r.ins_ref_index or 0}"]
                else:
                    tokens = ["unknown"]
...
def _recompute_aggregates_from_components(r: "Record") -> None:
    ...
    uncov_origins = [c.uncov_origin for c in comps if c.uncov_origin]
    r.uncov_origin = uncov_origins[0] if uncov_origins else None
    # Customer
...
def prioritize(all_recs: List[Record], max_sites: int):
    ...
    remaining_slots = max_sites - sites_count
    # Add all available UNCOVERseq rows up to remaining max_sites limit
    if remaining_slots > 0:
        pool_add = uncov_pool_all[:remaining_slots]
    else:
        pool_add = []
...
def build_dedup_nonmerged_prioritized_rows(prioritized: list) -> list:
    ...
    for r in prioritized:
        for c in (r.components or []):
            ...
            if comp_is_on(c):
                # ON target: include origin prefix for consistency with merge_sources naming
                if c.source == "UNCOVERseq":
                    origin_prefix = c.uncov_origin if c.uncov_origin else "UNCOVERseq"
                    orig_name = f"{origin_prefix}_OnT"
                elif c.source == "customer":
                    orig_name = "customer_OnT"
                elif c.source == "insilico_pop":
                    orig_name = "insilico_OnT"
                ...
                origin_prefix = c.uncov_origin if c.uncov_origin else "UNCOVERseq"
                orig_name = f"{origin_prefix}_Tier{t}_{x}"
            elif c.source == "customer":
...
def main(argv=None):
    ...
    p.add_argument("--customer", type=str, help="Customer CSV/TSV with headers: chr,start,stop,strand (+ optional fields)")
    ...
    # Load sources
    df_customer = load_customer_csv(args.customer) if args.customer else pd.DataFrame()
    ...
    # Apply UMI filtering to keep only rows with "Avg. % On-Target UMI" > 0.5
    df_customer = filter_by_umi_threshold(df_customer, threshold=0.5)
    df_uncov = filter_by_umi_threshold(df_uncov, threshold=0.5)
    df_pop = filter_by_umi_threshold(df_pop, threshold=0.5)
    df_ref = filter_by_umi_threshold(df_ref, threshold=0.5)
