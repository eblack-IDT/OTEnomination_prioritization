...
Two UNCOVERseq inputs with equal ranks; no customer or in-silico inputs.

Per requirements:
- Both UNCOVERseq inputs MUST have headers: chr, start, stop, strand (strand required).
- Both files are treated with equal priority: __input_order__ is 0-based within each file;
  for sites shared between files, the minimum order (best rank) is used.
...
- --max_sites sets per-output cap for part1 and part2 (default: 200).

Key outputs (tab-delimited, 0-based coordinates):
 1) *_part1.bed → Tier 1-3 AND Reproducibility == 3
 2) *_part2.bed → Tier 1-3 AND Reproducibility == 2
 3) all_candidates_full.bed → all candidates
 4) *_merge_mapping.tsv → map of merged components → final name

Additional outputs:
 5) *_part1_assay_mapping.csv
 6) *_part2_assay_mapping.csv
...
Aggregation rules:
 ...
 - Output 'Annot.' is derived from UNCOVERseq 'Annotation'
 - Component ordering for aggregation: UNCOVERseq input 1 → UNCOVERseq input 2
"""
...
# ------------------------------
def load_customer_csv(path: Optional[str]) -> pd.DataFrame:
    if not path:
    ...
    for col in ["chr","start","stop","strand"]:
        if col not in df.columns:
            raise ValueError(f"Customer: missing required column '{col}' after normalization")
    ...
    if "Name" in df.columns:
        ...
        df["raw_name"] = s_clean.fillna("customer")
    else:
        df["cust_name"] = ""
        df["raw_name"] = "customer"
    df["source"] = "customer"
    return df
...
def load_uncoverseq_csv(path: Optional[str]) -> pd.DataFrame:
    ...
    _ensure_str_col(df, "Site Type", "")
    is_on = df["Site Type"].str.contains(r"\bon\b", case=False, na=False)
    # UNCOV LD (for filtering < 7 later)
...
class Component:
    ...
    uncov_source_label: Optional[str] = None  # "iA" for input1, "SNP1" for input2
    ...
    def display_token(self) -> str:
        ...
        if s == "customer":
            ...
            lbl = self.uncov_source_label or ""
            prefix = f"{lbl}_" if lbl else ""
            return f"{prefix}Tier{y}_{x}"
        elif s == "insilico_pop":
...
class Record:
    ...
    # Example: ["Tier3_3","customer_OT1","insilico_pop65","insilico_ref44"]
    name_list: List[str]
...
def merge_sources(df_customer: pd.DataFrame,
    ...
    def add_row(row: pd.Series, label: str):
        ...
        if label == "UNCOVERseq":
            ...
            comp.uncov_source_label = str(row.get("__uncov_label__", "") or "")
        ...
        if r.is_on:
            if not multiple_on:
                ...
                if r.uncov_is_on:
                    ...
                    on_count_cust += 1
                    tokens = [f"OnT_customer_{on_count_cust}"]
                else:
                    tokens = ["OnT"]
        else:
            if r.uncov_present:
                ...
                # Build ordered, deduplicated label prefix (iA before SNP1 by insertion order)
                _seen_lbl: dict = {}
                for _c in r.components:
                    if _c.source == "UNCOVERseq" and _c.uncov_source_label:
                        _seen_lbl[_c.uncov_source_label] = None
                _lbl_prefix = ("_".join(_seen_lbl.keys()) + "_") if _seen_lbl else ""
                base = f"{_lbl_prefix}Tier{tier_val}_{x}"
                tokens = [base]
                if r.customer_present and not r.customer_is_on:
                    cx = r.customer_nonon_index if r.customer_nonon_index is not None else 0
                    tokens.append(f"customer_OT{cx}")
                if r.ins_pop_present:
                    tokens.append(f"insilico_pop{r.ins_pop_index or 0}")
                if r.ins_ref_present:
                    tokens.append(f"insilico_ref{r.ins_ref_index or 0}")
            elif r.customer_present:
                if r.customer_is_on:
                    tokens = ["OnT"]
                else:
                    if r.customer_name:
                        base = f"Customer_{r.customer_name}"
                    else:
                        x = r.customer_nonon_index if r.customer_nonon_index is not None else 0
                        base = f"Customer_OT{x}"
                    tokens = [base]
                    if r.ins_pop_present:
                        tokens.append(f"insilico_pop{r.ins_pop_index or 0}")
                    if r.ins_ref_present:
                ...
                if r.ins_pop_present and r.ins_ref_present:
                    tokens = [f"Insilico_pop{r.ins_pop_index or 0}", f"insilico_ref{r.ins_ref_index or 0}"]
                elif r.ins_pop_present:
                    tokens = [f"Insilico_pop{r.ins_pop_index or 0}"]
                elif r.ins_ref_present:
                    tokens = [f"Insilico_ref{r.ins_ref_index or 0}"]
                else:
                    tokens = ["unknown"]
...
def prioritize(all_recs: List[Record], max_sites: int):
    """
    Order:
      1) Customer (original order)
      2) UNCOVERseq after LD<7
      3) In-silico: LD block (cap 100) then Tier 1–3 block
    """
    ...
    add_in_order(cust_block, key=lambda r: (r.cust_min_order if r.cust_min_order is not None else 10**12))
    if len(selected) >= max_sites:
        return selected, []

    # 2) UNCOVERseq block (apply LD < 7)
    ...
    uncov_pool_all.sort(key=lambda r: (r.uncov_min_order if r.uncov_min_order is not None else 10**12))
    tier13 = [r for r in uncov_pool_all if (r.uncov_tier is not None and 1 <= r.uncov_tier <= 3)]
    repro_gt1 = [r for r in tier13 if (r.uncov_repro is not None and r.uncov_repro > 1)]
    repro_eq3 = [r for r in tier13 if (r.uncov_repro == 3)]

    sites_count = len(selected)
    if len(tier13) + sites_count > max_sites:
        pool_add = repro_eq3 if (len(repro_gt1) + sites_count > max_sites) else repro_gt1
    elif len(tier13) + sites_count < 50:
        pool_add = uncov_pool_all if (len(uncov_pool_all) + sites_count < max_sites) else uncov_pool_all[:100]
    else:
        pool_add = tier13
    add_in_order(pool_add)
    if len(selected) >= max_sites:
        return selected, []

    # 3) In-silico
    remaining = [r for r in all_recs if (r.chr, r.start, r.stop, r.strand) not in seen]
    # Phase A: LD criteria (LD < 3) OR (LD == 3 and gam_region == Exon)
    ins_ld_pool = [r for r in remaining
                   if (r.ins_pop_present or r.ins_ref_present) and
                      ((r.ins_ld_min is not None and r.ins_ld_min < 3) or r.ins_ld3_exon)]
    ins_ld_pool.sort(key=lambda r: (
    ...
    remaining_slots = max_sites - len(selected)
    cap = min(100, remaining_slots)
    add_in_order(ins_ld_pool, limit=cap)
    if len(selected) >= max_sites:
        return selected, []

    # Phase B: Tier score list (Tier 1–3)
    remaining = [r for r in all_recs if (r.chr, r.start, r.stop, r.strand) not in seen]
    ins_tier_pool = [r for r in remaining
                     if (r.ins_pop_present or r.ins_ref_present) and
                        (r.ins_tier_min is not None and 1 <= r.ins_tier_min <= 3)]
    ins_tier_pool.sort(key=lambda r: (
        r.ins_tier_min if r.ins_tier_min is not None else 9,
        r.ins_ld_min if r.ins_ld_min is not None else 1e9,
        r.ins_mar_rank_min if r.ins_mar_rank_min is not None else 2,
        min([x for x in [r.ins_pop_index, r.ins_ref_index] if x is not None], default=10**9)
    ))
    add_in_order(ins_tier_pool)

    # Build excluded (stable source-aware order)
    all_by_source_order = []
    all_by_source_order.extend(sorted([r for r in all_recs if r.customer_present],
                                      key=lambda r: (r.cust_min_order if r.cust_min_order is not None else 10**12)))
...
def build_dedup_nonmerged_prioritized_rows(prioritized: list) -> list:
    ...
    'name' is the ORIGINAL component label, not the merged BED name:
      ...
      - customer:  'Customer_{cust_name}' if available else 'Customer_OT{index}'
    ...
    for r in prioritized:
        for c in (r.components or []):
            ...
            if comp_is_on(c):
                ...
                lbl = getattr(c, "uncov_source_label", "") or ""
                prefix = f"{lbl}_" if lbl else ""
                orig_name = f"{prefix}Tier{t}_{x}"
...
def _component_sort_key(c: Component) -> int:
    ...
    Ordering priority for aggregation:
      UNCOVERseq (0) < insilico (1) < customer (2)
...
def write_assay_mapping(path: str, recs: List[Record]) -> int:
    ...
    Values are ';'-joined across merged originals in order:
      UNCOVERseq -> insilico -> customer.
    Output 'Annot.' is sourced from UNCOVERseq 'Annotation' and insilico 'gam_region'; customer contributes 'NONE'.
...
def main(argv=None):
    p = argparse.ArgumentParser(
        description="Nomination site prioritization — two UNCOVERseq inputs, part1/part2 outputs"
    )
    p.add_argument("--uncoverseq1", required=True, type=str,
                   help="First UNCOVERseq CSV/TSV with headers: chr,start,stop,strand (+ Tier/Reproducibility/Site Type)")
    p.add_argument("--uncoverseq2", required=True, type=str,
                   help="Second UNCOVERseq CSV/TSV with headers: chr,start,stop,strand (+ Tier/Reproducibility/Site Type)")
    p.add_argument("--dedupe", choices=["by_coord", "none"], default="by_coord",
                   help="Deduplication mode (default: by_coord)")
    p.add_argument("--out_prefix", type=str, default=None,
                   help="Path + filename prefix for outputs")
    p.add_argument("--outdir", type=str, default=None,
                   help="Output directory (used only if --out_prefix not provided)")
    p.add_argument("--verbose", action="store_true",
                   help="Echo banner with resolved paths, settings, and input counts")
    p.add_argument("--max_sites", type=int, default=MAX_SITES,
                   help="Maximum number of sites written to each output part (default: 200)")
    ...
    p.add_argument("--target_distance", type=int, default=500,
                   help="Gap between adjacent targets (bp) for proximity merge (default: 500)")
    ...
    p.add_argument("--target_flank", type=int, default=40,
                   help="Flank length subtracted when deriving max_merge_size (default: 40)")
    ...
    p.add_argument("--merge_name_delim", type=str, default="__",
                   help="Delimiter used when joining names during merge (default: '__')")
    ...
    if args.out_prefix:
        ...
        part1_path   = os.path.join(out_dir, f"{base}_part1.bed")
        part2_path   = os.path.join(out_dir, f"{base}_part2.bed")
        ...
        dedup_nm_path = os.path.join(out_dir, f"{base}_dedup_nonmerged_prioritized_sites.bed")
        am_part1     = os.path.join(out_dir, f"{base}_part1_assay_mapping.csv")
        am_part2     = os.path.join(out_dir, f"{base}_part2_assay_mapping.csv")
        am_all       = os.path.join(out_dir, f"{base}_all_candidates_assay_mapping.csv")
    else:
        out_dir = args.outdir or "."
        os.makedirs(out_dir, exist_ok=True)
        part1_path   = os.path.join(out_dir, "part1.bed")
        part2_path   = os.path.join(out_dir, "part2.bed")
        full_path    = os.path.join(out_dir, "all_candidates_full.bed")
        mapping_path = os.path.join(out_dir, "merge_mapping.tsv")
        dedup_nm_path = os.path.join(out_dir, "dedup_nonmerged_prioritized_sites.bed")
        am_part1     = os.path.join(out_dir, "part1_assay_mapping.csv")
        am_part2     = os.path.join(out_dir, "part2_assay_mapping.csv")
        am_all       = os.path.join(out_dir, "all_candidates_assay_mapping.csv")

    # ---- Load both UNCOVERseq inputs with equal ranks ----
    # Each file's __input_order__ starts at 0 independently (equal priority).
    # Offset file2's UNCOV_nonon_index by the number of non-ON-target sites in file1
    # to guarantee unique site names across files.
    df_uncov1 = load_uncoverseq_csv(args.uncoverseq1)
    df_uncov2 = load_uncoverseq_csv(args.uncoverseq2)

    n1_nonon = int((df_uncov1["UNCOV_nonon_index"] > 0).sum()) if not df_uncov1.empty else 0
    if n1_nonon > 0 and not df_uncov2.empty:
        mask2 = df_uncov2["UNCOV_nonon_index"] > 0
        df_uncov2 = df_uncov2.copy()
        df_uncov2.loc[mask2, "UNCOV_nonon_index"] = (
            df_uncov2.loc[mask2, "UNCOV_nonon_index"].astype(int) + n1_nonon
        )

    # Tag each input so targets carry their source label in the name
    df_uncov1["__uncov_label__"] = "iA"
    df_uncov2["__uncov_label__"] = "SNP1"
    df_uncov = pd.concat([df_uncov1, df_uncov2], ignore_index=True)
    ...
    if args.verbose:
        ...
        print(f"Max sites per part: {args.max_sites}")
        print(f"Merge strategy: {args.merge_targets}")
        if args.merge_targets == "proximity":
            derived_mms = (
                (args.max_insert - args.target_flank - 15)
                if args.max_merge_size is None
                else args.max_merge_size
            )
        ...
        print()
        print("Output paths:")
        print(f"  part1:                      {part1_path}")
        print(f"  part2:                      {part2_path}")
        print(f"  full list:                  {full_path}")
        print(f"  mapping:                    {mapping_path}")
        print(f"  dedup_nonmerged_prioritized:{dedup_nm_path}")
        print(f"  assay map part1:            {am_part1}")
        print(f"  assay map part2:            {am_part2}")
        print(f"  assay map all:              {am_all}")
        print()
        print("Input summary:")
        print(f"  UNCOVERseq 1 rows: {len(df_uncov1)}")
        print(f"  UNCOVERseq 2 rows: {len(df_uncov2)}")
        print(f"  Combined rows:     {len(df_uncov)}")
        print(sep)
        print()

    # ---- Merge sources (no customer, no in-silico) ----
    all_records, _ = merge_sources(
        pd.DataFrame(), df_uncov, pd.DataFrame(), pd.DataFrame(), args.dedupe
    )
    ...
    # ---- Filter into part1 and part2 ----
    # part1: Tier 1-3 AND Reproducibility == 3
    # part2: Tier 1-3 AND Reproducibility == 2
    # Within each part, sort by Tier ascending then original input order.
    def _sort_key(r: Record):
        return (
            r.uncov_tier if r.uncov_tier is not None else 9,
            r.uncov_min_order if r.uncov_min_order is not None else 10 ** 12,
        )

    part1 = sorted(
        [r for r in all_records
         if r.uncov_tier is not None and 1 <= r.uncov_tier <= 3 and r.uncov_repro == 3],
        key=_sort_key,
    )[:max(0, int(args.max_sites))]
    part2 = sorted(
        [r for r in all_records
         if r.uncov_tier is not None and 1 <= r.uncov_tier <= 3 and r.uncov_repro == 2],
        key=_sort_key,
    )[:max(0, int(args.max_sites))]

    # ---- Write outputs ----
    n_part1 = write_bed(part1_path, part1)
    n_part2 = write_bed(part2_path, part2)
    n_full  = write_bed(full_path, all_records)
    write_merge_mapping(mapping_path, all_records, part1)
    
    # Dedup nonmerged: original per-component coordinates from part1 + part2
    dedup_nm_rows = build_dedup_nonmerged_prioritized_rows(part1 + part2)
    n_dedup_nm = write_bed6(dedup_nm_path, dedup_nm_rows)

    n_am_part1 = write_assay_mapping(am_part1, part1)
    n_am_part2 = write_assay_mapping(am_part2, part2)
    n_am_all   = write_assay_mapping(am_all,   all_records)

    print(f"Wrote {n_part1:>4} lines (part1, Tier1-3 Repro==3):     {part1_path}")
    print(f"Wrote {n_part2:>4} lines (part2, Tier1-3 Repro==2):     {part2_path}")
    print(f"Wrote {n_full:>4} lines (all candidates):                 {full_path}")
    print(f"Wrote {n_dedup_nm:>4} lines (dedup nonmerged prioritized): {dedup_nm_path}")
    print(f"Wrote mapping: {mapping_path}")
    print(f"Wrote {n_am_part1:>4} rows: {am_part1}")
    print(f"Wrote {n_am_part2:>4} rows: {am_part2}")
